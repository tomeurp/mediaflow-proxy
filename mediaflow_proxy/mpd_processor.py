...
def _is_subtitle_profile(profile: dict) -> bool:
    mime_type = (profile.get("mimeType") or "").lower()
    return "text" in mime_type or "vtt" in mime_type or "ttml" in mime_type
...
def build_hls(
    ...
    for profile in mpd_dict["profiles"]:
        ...
        elif _is_subtitle_profile(profile):
            subtitle_profiles[profile["id"]] = (profile, playlist_url)
    ...
    # Subtitle tracks. DASH text/vtt or TTML AdaptationSets are exposed as HLS
    # subtitle renditions so players can discover them from the master playlist.
    # The per-profile playlist is generated by build_hls_playlist().
    if subtitle_profiles:
        sorted_subtitles = sorted(
            subtitle_profiles.values(),
            key=lambda pv: (pv[0].get("lang") or "und", pv[0].get("id") or ""),
        )
        seen_sub_langs = set()
        for profile, playlist_url in sorted_subtitles:
            lang = profile.get("lang") or "und"
            dedupe_key = (lang, profile.get("label") or "")
            if dedupe_key in seen_sub_langs:
                continue
            seen_sub_langs.add(dedupe_key)
            label = profile.get("label") or lang or "Subtitles"
            name = f"Subtitles {label}" if label != "und" else "Subtitles"
            hls.append(
                f'#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="subs",NAME="{name}",DEFAULT=NO,AUTOSELECT=YES,LANGUAGE="{lang}",URI="{playlist_url}"'
            )
    ...
    for profile, playlist_url in video_profiles.values():
        ...
        subtitles_attr = ',SUBTITLES="subs"' if subtitle_profiles else ""
        ...
        if is_ts_mode:
            hls.append(
                f'#EXT-X-STREAM-INF:BANDWIDTH={profile["bandwidth"]},RESOLUTION={profile["width"]}x{profile["height"]},CODECS="{combined_codecs}"{audio_attr}{subtitles_attr}'
            )
        else:
            hls.append(
                f'#EXT-X-STREAM-INF:BANDWIDTH={profile["bandwidth"]},RESOLUTION={profile["width"]}x{profile["height"]},CODECS="{combined_codecs}",FRAME-RATE={profile["frameRate"]}{audio_attr}{subtitles_attr}'
            )
...
def build_hls_playlist(
    ...
    if profiles and all(_is_subtitle_profile(profile) for profile in profiles):
        profile = profiles[0]
        segments = profile.get("segments") or []
        if not segments and profile.get("textUrl"):
            total_duration = profile.get("mediaPresentationDuration")
            if isinstance(total_duration, str):
                # Import is avoided here; parsed subtitle profiles normally include segments
                # when requested via parse_segment_profile_id. Fall back to 1 second.
                duration = 1.0
            else:
                duration = float(total_duration or 1.0)
            segments = [{"media": profile["textUrl"], "extinf": duration}]
        extinf_values = [float(segment.get("extinf", 1.0)) for segment in segments] or [1.0]
        hls.extend(
            [
                f"#EXT-X-TARGETDURATION:{max(1, math.ceil(max(extinf_values)))}",
        ...
        for segment in segments:
            hls.append(f"#EXTINF:{float(segment.get('extinf', 1.0)):.3f},")
            hls.append(segment["media"])
        if not mpd_dict.get("isLive", False):
            hls.append("#EXT-X-ENDLIST")
        logger.info(f"Added {len(segments)} subtitle segments to HLS playlist")
        return "\n".join(hls)
